...
## 2026-08-28 — 교육 교재 '2. AI의_이해.md' 1.4절 및 '1-2. 교육과정.md' 강의 계획 최신화

### 교육 교재 개선
- **`1.교육자료/기본_AI 이해/2. AI의_이해.md` 1.4절 전면 개정**:
  - **1.3절과의 연결성 강화**: 딥러닝 계보에서 "기계 내부에서 무슨 일이 일어나는가?"라는 질문으로 자연스럽게 이어지도록 인트로 보강.
  - **인과관계 기반 3단계 스토리라인 구성**:
    1. 컴퓨터는 글자를 모른다 (모든 것은 숫자로 변환)
    2. 지식은 고정된 계산 공식이다 (가중치 & 요리 레시피 비유)
    3. 결과는 정답이 아닌 후보별 확률 점수다 (스팸 분류 vs LLM 다음 토큰 점수)
  - **1.5절로의 브릿지 형성**: "그렇다면 글자를 어떻게 숫자로 쪼갤까?"라는 질문을 유도하여 1.5절의 토큰화/자기회귀 단계로의 이해도 극대화.
- **`1.교육자료/기본_AI 이해/1-2. 교육과정.md` 강의 로드맵 동기화**:
  - 교재 2(`Part 0~5`) 신규 목차와 정합성을 맞추어 1일차 09:00~09:30 기초 지도 세부 항목(회사·모델·서비스, 분류형/생성형, 구독제 vs API 등) 명시.

---

## 2026-08-18 — 원격 저장소 이관 및 심화 교육자료 저장소 포함 (`.gitignore` 정비)

### 저장소 관리 및 거버넌스
- **원격 저장소 URL 이관**: `2026.09-kbs-vibe-coding` ➔ `2026.autoreport` 원격 저장소(`main` 브랜치)로 동기화 완료.
- **`.gitignore` 갱신 및 심화 교육자료 포함**:
  - `1.교육자료/심화_AI엔지니어링/` 폴더 내 심화 마크다운 교재 9종을 원격 저장소에 포함.
  - 불필요한 예외 규칙을 정리하여 저장소 일관성 확보.
